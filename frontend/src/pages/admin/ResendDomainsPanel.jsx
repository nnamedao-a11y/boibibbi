...
export default function ResendDomainsPanel({ hasApiKey }) {
  ...
  const handleAdd = async () => {
    ...
    if (!name || !name.includes('.')) {
      toast.error('Введите валидный домен, например bibi.cars');
      return;
    }
    setAdding(true);
    try {
      ...
      toast.success(`Домен ${name} добавлен. Теперь добавьте DNS-записи.`);
      setNewDomain('');
      setShowAdd(false);
      await fetchList();
      // Сразу раскрываем новый домен, чтобы admin видел DNS-записи
      if (created?.id) {
  ...
  const handleVerify = async (domainId) => {
    ...
    try {
      ...
      if (fresh?.status === 'verified') {
        toast.success(`✓ Домен ${fresh.name} верифицирован!`);
      } else {
        toast.info(`Status: ${fresh?.status || 'unknown'} — DNS ещё распространяется, попробуйте через 5-30 минут.`);
      }
      await fetchList();
    } catch (e) {
  ...
  const handleDelete = async (domain) => {
    if (!window.confirm(`Удалить домен "${domain.name}" из Resend? Все DNS-записи останутся, но домен исчезнет из списка.`)) return;
    ...
    try {
      ...
      toast.success(`Домен ${domain.name} удалён`);
                    ...
                    {!detail ? (
                      ...
                      <>
                        ...
                        <div className="rounded-lg bg-white border border-[#E4E4E7] px-3 py-2 text-[11px] text-[#71717A] leading-relaxed">
                          <b className="text-[#18181B]">Что делать:</b> скопируйте каждую запись в панель управления
                          DNS вашего регистратора (Cloudflare, Namecheap, GoDaddy и т.д.).
                          Для DKIM TXT-записей значение длинное — копируйте полностью.
                          Через 5-30 минут после добавления нажмите <b>Verify now</b>.
                        </div>
                      </>
                    )}
                  </div>
