...
const PROVIDER_STYLE = {
  resend:  { label: 'resend',  hint: 'Live: настроен Resend API. Бесплатный план: 3 000 писем/мес + 100/день.', chip: 'bg-emerald-50 text-emerald-700 ring-emerald-200' },
  smtp:    { label: 'smtp',    hint: 'Live: настроен SMTP (fallback от Resend).',                                chip: 'bg-emerald-50 text-emerald-700 ring-emerald-200' },
  dry_run: { label: 'dry_run', hint: 'Письма не отправляются — настройте Resend в /admin/integrations → Resend (re_xxx) или SMTP в Email.', chip: 'bg-amber-50 text-amber-700 ring-amber-200' },
};
...
export default function EmailOutboxPage({ embedded = false }) {
  ...
  const sendTest = async () => {
    ...
    try {
      ...
      if (ok && mode !== 'dry_run') {
        toast.success(`Email отправлен · provider=${mode}`);
      } else if (mode === 'dry_run') {
        toast.info(`Email залогирован в dry_run (Resend / SMTP не настроены)`);
      } else {
        toast.error(`Ошибка: ${err || 'unknown'}`);
      }
      await load();
    } catch (e) {
