...
export default function ResendWebhooksPanel({ hasApiKey }) {
  ...
  const handleCreate = async () => {
    ...
    if (!url.startsWith('http')) {
      toast.error('Endpoint URL должен быть https://...');
      return;
    }
    if (newEvents.size === 0) {
    ...
    try {
      ...
      toast.success('Webhook создан и подписной секрет сохранён в настройках');
      setShowAdd(false);
      await fetchList();
    } catch (e) {
  ...
  const handleDelete = async (wh) => {
    if (!window.confirm(`Удалить webhook ${wh.endpoint_url}? Resend перестанет слать события на этот URL.`)) return;
            ...
            <div className="flex items-start justify-between gap-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-[11.5px] text-[#71717A]">Подписной секрет уже сохранён в settings.</p>
                </div>
              </div>
