...
const PROVIDER_STYLE = {
  textbelt:      { label: 'textbelt (paid)',  hint: '',                                                          chip: 'bg-emerald-50 text-emerald-700 ring-emerald-200' },
  textbelt_free: { label: 'textbelt (free)',  hint: 'Бесплатный публичный ключ — 1 SMS/день/IP без регистрации.', chip: 'bg-sky-50 text-sky-700 ring-sky-200' },
  dry_run:       { label: 'dry_run',          hint: 'SMS отключён в /admin/integrations → SMS. Сообщения только логируются.', chip: 'bg-amber-50 text-amber-700 ring-amber-200' },
};
...
export default function SmsOutboxPage({ embedded = false }) {
  ...
  const sendTest = async () => {
    ...
    if (!phone) {
      toast.error('Введите номер в формате +XXXXXXXXX');
      return;
    }
    setSending(true);
    try {
      ...
      if (ok) {
        toast.success(`SMS отправлено · mode=${mode}`);
      } else if (mode === 'dry_run') {
        toast.info(`SMS залогировано в dry_run (канал выключен)`);
      } else {
        toast.error(`Ошибка отправки: ${err || 'unknown'}`);
      }
      await load();
    } catch (e) {
