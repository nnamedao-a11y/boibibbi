...
const PERMISSION_INFO = {
  full_access: {
    label: 'Full access',
    hint: 'Полный доступ: отправка писем + управление доменами / ключами / webhook’ами. Используйте только для admin-инструментов.',
    chip: 'bg-rose-50 text-rose-700 ring-rose-200',
  },
  sending_access: {
    label: 'Sending access',
    hint: 'Только отправка emails. Рекомендуется для production-сервисов (CRM, бэкенда BIBI Cars).',
...
export default function ResendApiKeysPanel({ hasApiKey }) {
  ...
  const handleCreate = async () => {
    ...
    if (!name) {
      toast.error('Введите название ключа (например: production-backend)');
      return;
    }
    setAdding(true);
    try {
      ...
      if (created?.token) {
        ...
        toast.success('Ключ создан — обязательно сохраните его сейчас!');
      } else {
  ...
  const handleDelete = async (key) => {
    if (!window.confirm(`Удалить API-ключ "${key.name}"? Все сервисы, которые его используют, перестанут работать.`)) return;
  ...
  if (!hasApiKey) {
    return (
      <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-[12.5px] text-amber-800 flex items-start gap-2">
        ...
        <span>Сначала сохраните основной Resend API Key выше.</span>
      </div>
    );
  }
      ...
      {showAdd && (
        <div className="rounded-xl border border-[#E4E4E7] bg-[#FAFAFA] p-3 space-y-3">
          ...
          <p className="text-[11px] text-[#71717A] leading-relaxed">
            {PERMISSION_INFO[newPermission].hint}
          </p>
          <div className="flex items-center gap-2">
            <button
              type="button"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={() => setCreatedKey(null)}>
          ...
          >
            <div className="flex items-start justify-between gap-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-[11.5px] text-[#71717A]">Resend покажет значение только ОДИН РАЗ.</p>
                </div>
              </div>
              <button
                type="button"
            ...
            <div className="rounded-xl border border-amber-200 bg-amber-50 px-3 py-2.5 text-[12px] text-amber-800 flex items-start gap-2">
              <AlertTriangle className="w-4 h-4 shrink-0 mt-0.5" />
              <span><b>Сохраните этот ключ сейчас.</b> После закрытия окна вы больше не увидите его значение и не сможете восстановить.</span>
            </div>
