...
const PROVIDER_CONFIG = {
  ...
  email: {
    ...
    color: '#EA4335',
    description:
      'Старый Email-канал через любой SMTP-сервер (Gmail / Outlook / Postmark / собственный SMTP). Используется как fallback, если Resend не настроен.',
    fields: [
  ...
  resend: {
    ...
    color: '#000000',
    description:
      'Современный email-API. Бесплатный план: 3 000 писем/мес + 100 писем/день. Получите re_xxx ключ на resend.com → API Keys. Домен отправителя должен быть верифицирован в resend.com → Domains (SPF + DKIM + DMARC), иначе письма вернут 403.',
    fields: [
      { key: 'apiKey', label: 'Resend API Key', type: 'password', placeholder: 're_xxxxxxxxxxxxxxxx' },
      { key: 'from',   label: 'From (verified sender)', type: 'text', placeholder: 'BIBI Cars <noreply@bibi.cars>' },
  ...
  sms: {
    ...
    color: '#0EA5E9',
    description:
      'SMS-уведомления через TextBelt. По умолчанию работает БЕСПЛАТНО (публичный ключ — 1 SMS/день с одного IP, без регистрации). Для безлимита введите ваш ключ с textbelt.com (~$0.01–0.10 за SMS).',
    fields: [
      {
        key: 'apiKey',
        label: 'TextBelt API Key (оставьте пустым для бесплатного режима)',
        type: 'password',
        placeholder: 'textbelt (free)  ИЛИ  ваш ключ с textbelt.com',
      },
    ],
    settings: [
