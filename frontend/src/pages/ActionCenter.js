          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-5" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div>
                ...
                <select value={draft.priority} onChange={(e) => setDraft({ ...draft, priority: e.target.value })} className="w-full mt-1 px-3 py-2 border border-[#E4E4E7] rounded-lg text-[13px]" data-testid="draft-priority">
                  {['critical', 'high', 'medium', 'low'].map((p) => <option key={p} value={p}>{p}</option>)}
                </select>
              </div>
              <div>
                <label className="text-[11px] uppercase tracking-wider font-bold text-[#71717A]">{t('w17_form_due')}</label>
                <input type="datetime-local" value={draft.due_at} onChange={(e) => setDraft({ ...draft, due_at: e.target.value ? new Date(e.target.value).toISOString() : '' })} className="w-full mt-1 px-3 py-2 border border-[#E4E4E7] rounded-lg text-[13px]" data-testid="draft-due" />
              </div>
            </div>
