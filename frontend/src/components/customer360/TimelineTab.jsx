    ...
    <div className="space-y-5" data-testid="customer360-timeline-tab">
      ...
      {available.length > 0 && (
        <div className="flex flex-wrap gap-2">
          ...
          {active.size > 0 && (
            <button onClick={() => setActive(new Set())} className="text-xs text-zinc-500 hover:text-zinc-900 underline">Сбросить</button>
          )}
        </div>
      )}
      ...
      {filtered.length === 0 && (
        <div className="section-card text-center py-12" data-testid="timeline-empty">
          ...
          <p className="text-[#71717A]">Нет событий по фильтру.</p>
        </div>
      )}
