    ...
    <div className="space-y-5" data-testid="ringostat-stats-panel">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0 flex-1">
          <h2 className="text-base sm:text-lg font-bold tracking-tight text-[#18181B]">
            Call performance · last {days} days
          </h2>
          <p className="text-[11.5px] text-[#71717A] mt-0.5">
            Live KPI rollup across all managers and all directions. Auto-refresh 30 s.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <KPI
          ...
          icon={Phone}
          hint={`${fmt(totals.inbound)} inbound · ${fmt(totals.outbound)} outbound`}
          testid="kpi-total"
        />
        <KPI
          label="Answered"
          ...
          icon={ArrowDownLeft}
          accent="good"
          hint={`avg ${fmtDuration(overview?.avg_duration_sec)}`}
          testid="kpi-answered"
        />
        <KPI
          label="Missed"
          ...
          hint={(totals.missed || 0) ? 'needs callback' : 'none'}
          testid="kpi-missed"
        />
        <KPI
          label="Answer rate"
      ...
      <div className="bg-white border border-[#E4E4E7] rounded-2xl p-4 sm:p-5">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="ml-auto text-[10px] uppercase tracking-wider text-[#A1A1AA]">
            {managers.length} agents
          </span>
        </div>
        {managers.length === 0 ? (
          <p className="text-xs text-[#A1A1AA] py-6 text-center">
            {loading ? 'Loading…' : 'No managers received calls in this window.'}
          </p>
        ) : (
            ...
            <table className="w-full text-[12px] tabular-nums">
              <thead className="text-[10.5px] uppercase tracking-wider text-[#71717A]">
                <tr className="border-b border-[#F4F4F5]">
                  <th className="text-left font-semibold px-2 py-2">Manager</th>
                  <th className="text-left font-semibold px-2 py-2">Ext.</th>
                  <th className="text-right font-semibold px-2 py-2">Total</th>
                  <th className="text-right font-semibold px-2 py-2">Answered</th>
                  <th className="text-right font-semibold px-2 py-2">Missed</th>
                  <th className="text-right font-semibold px-2 py-2">Answer&nbsp;%</th>
                  <th className="text-right font-semibold px-2 py-2">Avg&nbsp;dur</th>
                  <th className="text-left font-semibold px-2 py-2 hidden md:table-cell">Last call</th>
                </tr>
              </thead>
              <tbody>
                {managers.map((m) => {
