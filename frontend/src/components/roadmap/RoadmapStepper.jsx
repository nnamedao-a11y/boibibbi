 ...
 *   stageTemplate      — list of `{key, label_en, label_ru, label_bg, ...}`
 *   lang               — currently-active language code (en/bg/ru/uk)
...
const STATUS_META = {
  pending: {
    ...
    label: { en: 'Pending', ru: 'Ожидает', bg: 'Очаква', uk: 'Очікує' },
  },
  in_progress: {
    ...
    label: { en: 'In progress', ru: 'В работе', bg: 'В процес', uk: 'В роботі' },
  },
  done: {
    ...
    label: { en: 'Done', ru: 'Готово', bg: 'Готово', uk: 'Готово' },
  },
  completed: {
    pill: 'bg-emerald-100 text-emerald-700 border-emerald-200',
    ring: 'border-emerald-300 bg-emerald-50',
  ...
  blocked: {
    ...
    label: { en: 'Blocked', ru: 'Блокировано', bg: 'Блокирано', uk: 'Заблоковано' },
  },
  skipped: {
    ...
    icon: CaretRight,
    iconClass: 'text-zinc-400',
    label: { en: 'Skipped', ru: 'Пропущено', bg: 'Пропуснат', uk: 'Пропущено' },
  },
};

const pickLabel = (stage, lang) => {
  if (!stage) return '';
  const order = [lang, 'en', 'bg', 'ru'];
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[11px] uppercase tracking-wider font-bold text-amber-700">
                  {lang === 'ru' ? 'Текущий этап' : lang === 'bg' ? 'Текущ етап' : lang === 'uk' ? 'Поточний етап' : 'Current stage'}
                </p>
                ...
                {cur.eta && (
                  <p className="text-xs text-zinc-600 mt-2">
                    {lang === 'ru' ? 'Ожидаемый срок' : lang === 'bg' ? 'Очакван срок' : 'Expected'}: <span className="font-medium text-zinc-900">{fmtDate(cur.eta) || cur.eta}</span>
                  </p>
                )}
              </div>
            </div>
