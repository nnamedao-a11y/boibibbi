...
def build_default_stages() -> List[Dict[str, Any]]:
    """Return a fresh copy of the canonical 7-stage list with empty progress fields."""
    out: List[Dict[str, Any]] = []
    for tpl in DEFAULT_STAGES:
        out.append({
            **tpl,
            "status": "pending",
            "started_at": None,
...
async def create_roadmap(
    *,
    ...
    initial_stage: str = "vehicle_found",
    created_by: Optional[str] = None,
    created_by_email: Optional[str] = None,
) -> Dict[str, Any]:
    """Create a brand-new roadmap with the canonical 7 stages.

    ``initial_stage`` lets the caller bootstrap the roadmap at a more
    advanced point than ``vehicle_found`` (e.g. when the order was
    created from an already-paid invoice, the first two stages can be
    pre-marked as done by passing ``initial_stage='delivery_europe'``).
    """
    db = get_db()
    now = _now()
    ...
    if initial_stage and initial_stage != "vehicle_found":
        for s in stages:
    ...
    doc = {
        ...
        "title": title or (vehicle or {}).get("name") or "Vehicle roadmap",
        "vehicle": vehicle or {},
        "stages": stages,
...
async def update_stage(
    ...
    *,
    ...
    """Update a single stage's status / eta / sla / note. Recalculates rolling summary."""
    db = get_db()
