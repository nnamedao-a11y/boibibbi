...
def parse_detail(html: str, url: str) -> Optional[Dict[str, Any]]:
    ...
    # ─── PRIMARY: Schema.org JSON-LD (lemon-cars renders client-side, so
    # body text is empty in raw HTML — all hard data lives in <script
    # type="application/ld+json"> blocks: Vehicle, Product, Offer).
    # ───────────────────────────────────────────────────────────────────
    try:
        vehicle_ld: Dict[str, Any] = {}
        ...
        for s in soup.find_all("script", type="application/ld+json"):
            ...
            for o in objs:
                t = (o.get("@type") if isinstance(o, dict) else None) or ""
                t_norm = t if isinstance(t, str) else (t[0] if isinstance(t, list) and t else "")
                if t_norm == "Vehicle":
                    vehicle_ld = o
                elif t_norm == "Product":
                    product_ld = o
                # Nested offer inside Vehicle
                if isinstance(o, dict) and isinstance(o.get("offers"), dict):
                    offer_ld = o["offers"]

        if vehicle_ld:
            # name = "2017 NISSAN ARMADA SV"
            name = (vehicle_ld.get("name") or "").strip()
            if name:
                out["title"] = name
                # Try to parse year + make from the name as last-resort
                m = re.match(r"^\s*(\d{4})\s+([A-Z][A-Za-z\-]+)\s+(.+)$", name)
                if m:
                    out["year"] = out["year"] or int(m.group(1))
                    out["make"] = out["make"] or m.group(2).upper()
                    out["model"] = out["model"] or m.group(3).strip().upper()
            if vehicle_ld.get("brand"):
                br = vehicle_ld["brand"]
                out["make"] = out["make"] or (br.get("name") if isinstance(br, dict) else str(br)).upper()
            if vehicle_ld.get("model"):
                md = vehicle_ld["model"]
                out["model"] = out["model"] or (md.get("name") if isinstance(md, dict) else str(md)).upper()
            ymd = vehicle_ld.get("vehicleModelDate") or vehicle_ld.get("modelDate")
            if ymd:
                try:
                    out["year"] = out["year"] or int(str(ymd)[:4])
                except Exception:
                    pass
            eng = vehicle_ld.get("vehicleEngine") or {}
            if isinstance(eng, dict):
                et = eng.get("engineDisplacement") or eng.get("engineType")
                if et:
                    try:
                        out["engine_volume"] = out["engine_volume"] or float(str(et).replace(",", "."))
                    except Exception:
                        pass

        # Offer (price)
        if offer_ld and isinstance(offer_ld, dict):
            price = offer_ld.get("price")
            cur = (offer_ld.get("priceCurrency") or "USD").upper()
            if price:
                try:
                    val = float(str(price).replace(",", "."))
                    if cur == "USD":
                        out["current_bid_usd"] = out["current_bid_usd"] or val
                except Exception:
                    pass

        # Product → images
        if product_ld and not out.get("image"):
            ...
            if isinstance(imgs, list) and imgs:
                ...
                for i in imgs:
                    ...
                    elif i.startswith("/"): i = BASE + i
                    norm.append(i)
                out["images"] = norm
                out["image"] = norm[0] if norm else None
    except Exception as e:
        logger.debug(f"[lemon] JSON-LD parse failed for {url[:80]}: {e}")
    ...
    # ─── Fallback: og:title / og:image meta tags ───
    if not out.get("title"):
        ogt = soup.find("meta", property="og:title")
        if ogt and ogt.get("content"):
            out["title"] = _clean(ogt["content"])
    if not out.get("image"):
        ogi = soup.find("meta", property="og:image")
        if ogi and ogi.get("content"):
            out["image"] = ogi["content"]
            out["images"] = out["images"] or [out["image"]]
