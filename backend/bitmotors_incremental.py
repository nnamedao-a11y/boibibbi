...
class BitmotorsIncrementalSync:
    ...
    async def run_once(self, pages: Optional[int] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # Persist run history (P1.3 — was silently swallowing errors, now logs)
            try:
                if self.db is not None:
                    ...
                    logger.warning("[incremental] db handle is None — run history NOT persisted")
            except Exception as e:
                logger.error(f"[incremental] failed to persist run history: {e}")
