...
# ──────────────────────────────────────────────────────────────────────
# Deployment-resilience: auto-generate & persist a JWT_SECRET if env is
# missing/placeholder. Persists to /app/backend/.jwt_secret so the same
# value is reused across restarts (tokens issued before restart stay
# valid). This prevents the "Request failed with status code 500 /
# JWT_SECRET not configured — refusing to issue token" error from ever
# reappearing after a redeploy where .env got recreated without the key.
# ──────────────────────────────────────────────────────────────────────
    ...
    try:
        if os.path.exists(_jwt_secret_file):
            with open(_jwt_secret_file, "r", encoding="utf-8") as _fh:
                _persisted = _fh.read().strip()
            if _persisted and _persisted != JWT_DEFAULT_PLACEHOLDER:
                JWT_SECRET = _persisted
                logger.warning("[security] JWT_SECRET env not set — loaded persisted secret from .jwt_secret")
        if not JWT_SECRET or JWT_SECRET == JWT_DEFAULT_PLACEHOLDER:
            ...
            try:
                ...
                logger.error(f"[security] could not persist JWT_SECRET to {_jwt_secret_file}: {_e}")
            logger.warning("[security] JWT_SECRET auto-generated & persisted (set JWT_SECRET in .env to override)")
    except Exception as _e:
        # Last-resort fallback so service can still issue tokens.
        if not JWT_SECRET or JWT_SECRET == JWT_DEFAULT_PLACEHOLDER:
            JWT_SECRET = _secrets.token_urlsafe(64)
        logger.error(f"[security] JWT_SECRET bootstrap failed ({_e}) — using in-memory secret")
